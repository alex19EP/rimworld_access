...
namespace RimWorldAccess
{
    ...
    public static class InfoCardState
    {
        ...
        public static void ActivateItem()
        {
            ...
            // For expandable items that aren't expanded, expand them
            if (item.IsExpandable && !item.IsExpanded)
            {
                Expand();
                return;
            ...
            // For already-expanded items, do nothing (don't re-trigger OnActivate)
            if (item.IsExpandable && item.IsExpanded)
            {
                SoundDefOf.ClickReject.PlayOneShotOnCamera();
        ...
        private static void AnnounceCurrentSelection()
        {
            try
            {
                ...
                string announcement = $"{label}{stateIndicator}.{levelSuffix} {position} of {total}.";
